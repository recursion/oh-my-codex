...
**Native role-routing rule:** When the native surface exposes `agent_type`, set it to an installed OMX role. On `role_routing_unavailable`, do not fabricate it: the OMX adapted role-pass protocol is provenance only and does not apply the role TOML model or reasoning effort. Record the pre-validated intent, keep `requested_route` separate from `observed_route`, and use an OMX Team worker when exact routing is required; otherwise report the mismatch or blocker.
