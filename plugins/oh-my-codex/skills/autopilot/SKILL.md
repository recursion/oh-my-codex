...
- **On ralplan -> ultragoal**: only after `ralplan_consensus_gate.complete:true`, with tracker-backed typed-native review evidence, explicitly plan-bound authenticated adapted receipts, or validated attached-tmux `omx_team` evidence for `ralplan_architect_review.agent_role:"architect"` and then `ralplan_critic_review.agent_role:"critic"`, both approving and sequential. `codex_exec` or artifact-only approvals are trace evidence but not lane proof. Set `current_phase:"ultragoal"` and persist the plan/test-spec paths under `handoff_artifacts.ralplan`.
