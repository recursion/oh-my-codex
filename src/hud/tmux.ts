...
function splitOutputMatchesPaneId(rawOutput: string, paneId: string): boolean {
  return parseExactTmuxAuthorityScalar(rawOutput) === paneId;
}
...
export function createHudWatchPane(
  ...
  const targetBefore = canonicalTargetPaneId ? readCanonicalPaneIdSnapshot(execTmuxSync, canonicalTargetPaneId) : null;
  if (!globalBefore || (canonicalTargetPaneId && !targetBefore)) return null;
  ...
  const args = [
    ...
    ...(canonicalTargetPaneId ? ['-t', canonicalTargetPaneId] : []), '-c', cwd, '-P', '-F', '#{pane_id}', hudCmd,
  ];
  ...
  splitArgs[splitArgs.length - 1] = writeHudSplitOperationMarkedCommand(hudCmd, operationMarker);
  try {
    ...
    const splitOutput = execTmuxSync(splitArgs);
