...
export function buildRalphAppendInstructions(
  ...
  return [
    ...
    '- When it does not (`role_routing_unavailable`, for example a Codex App `spawn_agent` surface exposing only `task_name`, `message`, and `fork_turns`), do not fabricate `agent_type`; the OMX adapted role-pass protocol is provenance only and does not apply the role TOML model or reasoning effort.',
    '- For an adapted spawn, record a pre-validated role intent in the OMX subagent ledger, never fake the role via a prompt label, and record `requested_route` separately from runtime-evidenced `observed_route`.',
    '- When exact model or effort routing is required, use an OMX Team worker or another role-aware launcher; if none is available, report the route mismatch or blocker instead of claiming success.',
...
export async function ralphCommand(args: string[]): Promise<void> {
  ...
  await updateModeState('ralph', {
    ...
    native_subagent_policy: 'When the native surface exposes agent_type role routing, each parallel Codex dispatch must set agent_type to an installed OMX role and never omit it for OMX work. When role routing is unavailable, the adapted role-pass protocol is provenance only and does not apply the role TOML model or reasoning effort; record requested_route separately from observed_route. Use an OMX Team worker or another role-aware launcher when exact routing is required, otherwise report the mismatch or blocker. Phase completion must wait for active native subagent threads to finish.',
