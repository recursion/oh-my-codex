...
8. `deep-interview -> ralplan` is evidence-gated: answered or handoff-cleared question obligations alone do not complete deep-interview, and Autopilot ralplan handoff requires tracker-backed typed Architect and Critic lane evidence from preferred native lanes or validated OMX Team fallback lanes rather than `codex_exec`/artifact-only approvals
