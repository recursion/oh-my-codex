...
- Before substantive planning, reviewer delegation, HUD/runtime activation, or adapted role work, inspect the native task surface. Keyword routing state is not authority. When the surface exposes `agent_type`, use typed routing. When it reports `role_routing_unavailable`, do not fabricate `agent_type`; run `omx ralplan preflight --json` and, on `unsupported_documented_leader_proof`, stop by default. The sole exception is a reviewed, explicitly amended plan with a current acknowledged policy that passes `omx ralplan preflight --adapted-provenance --json`. Do not infer root authority from `session_id`, undocumented `thread_id`, session/pointer/transcript/cwd state, absent child data, or prompt labels.
...
- **OMX adapted role-pass protocol:** this is an exceptional plan-bound fallback, never a substitute for typed routing. OMX must pre-create the private launch capability and signed pre-launch authorization; the plugin verifies it and never self-mints a launch claim. Grant the explicit acknowledged policy, pass the adapted preflight, and preserve signed sequential Planner -> Architect -> Critic receipts. Team, `codex_exec`, child/self-parent, stale, foreign, replayed, or plan-drift attempts are denied from minting authorization; provenance does not prove the child role/model was enforced by Codex. Codex does not provide caller-bound PreToolUse-to-CLI transport, so malicious same-user processes that steal live credentials can race the one-use handoff.
...
0. **Pre-context intake (required before planning/execution loop starts)**:
   ...
   - When this is a Ralplan-originated handoff and native role routing is unavailable, use `omx ralplan preflight --json` and stop on its failure by default; do not create the snapshot or begin the loop. The sole exception is a reviewed, explicitly amended plan with a current acknowledged policy that passes `omx ralplan preflight --adapted-provenance --json` before intake.
...
7. **Architect verification** (native role):
   ...
   - On `role_routing_unavailable`, do not manufacture an Architect identity. Use typed routing where possible; otherwise proceed only through the reviewed, explicitly amended plan with a current acknowledged policy that passes `omx ralplan preflight --adapted-provenance --json`, followed by the signed Planner -> Architect -> Critic receipts described above.
...
- [ ] Architect verification passed: on a routing-capable surface via explicit `task(agent_type="architect", reasoning_effort="medium"...)` minimum. On role-routing-unavailable surfaces, normal Ralplan-originated work stops with the leader-proof diagnostic; the only exception is a reviewed, explicitly amended plan with a current acknowledged policy, successful adapted-provenance preflight, and signed sequential Planner -> Architect -> Critic receipts.
