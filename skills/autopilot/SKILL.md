...
- **On ralplan -> ultragoal**: only after `ralplan_consensus_gate.complete:true`, with tracker-backed typed `ralplan_architect_review.agent_role:"architect"` and `ralplan_architect_review.verdict:"approve"` recorded before tracker-backed typed `ralplan_critic_review.agent_role:"critic"` and `ralplan_critic_review.verdict:"approve"`; preferred `native_subagent` or validated `omx_team` provenance is required, while `codex_exec` or artifact-only approvals are trace evidence only. Set `current_phase:"ultragoal"` and persist the plan/test-spec paths under `handoff_artifacts.ralplan`.
