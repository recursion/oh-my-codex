...
**Native role-routing preflight:** Before substantive planning or reviewer delegation, inspect the native task surface. When the native surface exposes `agent_type` role routing, use typed native routing. When it reports `role_routing_unavailable`, do not fabricate `agent_type`. Default to stopping on `omx ralplan preflight --json` failure. The only fallback is an explicitly amended plan: grant its acknowledged policy with `omx ralplan adapted-provenance grant --plan <docs/plans/...> --acknowledge I_ACCEPT_AUTHENTICATED_ADAPTED_PROVENANCE --json`, then require `omx ralplan preflight --adapted-provenance --json`. Do not infer root identity from `session_id`, undocumented `thread_id`, session/pointer/transcript/cwd state, absence of child data, or a prompt label.

**Authenticated adapted rule:** The fallback is default-deny, plan-scoped, SHA/session/launch-bound, HMAC-signed, and expiring. OMX must pre-create the private launch capability and signed pre-launch authorization; the plugin verifies it and never self-mints a claim from environment values. The exact root PreToolUse invocation then mints a short-lived one-use authorization for each grant and role-intent; a durable leader attestation alone is insufficient. Only after grant and adapted preflight may the exact `omx ralplan role-intent write` path create sequential Planner -> Architect -> Critic child receipts. Team, `codex_exec`, child, self-parent, replayed, malformed, stale, foreign, or plan-drift attempts are denied from minting authorization. It authenticates provenance only; it does not make Codex enforce the requested child role/model/TOML. Codex does not provide caller-bound transport from PreToolUse to CLI, so a malicious same-user process that steals live credentials can still race the one-use handoff. Never reuse it outside the reviewed amended plan.

**Authenticated adapted review artifact:** After each Architect or Critic child
starts, write its normal review fields plus
`adapted_policy_id` and `adapted_receipt_signature`. Copy those exact values
from `.omx/state/subagent-tracking.json` at
`sessions[session_id].threads[thread_id].adapted_receipt.policy_id` and
`.signature` for that same role/thread; do not construct them from prose or
